...
- **Visible road/border generation, PlotSquared-style** (user-requested - the protection
  ...
  **User-reported, fixed - the spawn-chunk blind spot in that same "after they're attached"
  ...
  attaching the populator, which at that instant is precisely the pre-generated set (no
  hardcoded radius, so it self-adjusts to the server's own spawn-chunk settings). Deliberately
  scoped to a world that call actually created: `paintChunk` derives ground level from
  `getHighestBlockYAt`, which on an already-painted border column returns the *wall's* Y, so
  re-running it over a painted world would stack a new foundation and wall one block higher
  on every enable. Default `plot-gap` is 7
