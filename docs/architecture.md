...
- **User-requested: the Bans page no longer shows expired bans** - `WebPanelSnapshotService#applyRefresh` now filters `banDao.findRecent(...)` down to `Ban#isCurrentlyInEffect(now)` before building the page's entries, same shape as the mute list's existing expiry filter right below it in that method. This also fixed a real latent bug the request surfaced: `Ban#active` (the raw DB column) is only ever flipped by an explicit revoke/unban (`BanDao#revoke*`) - a temp ban simply reaching its own `expires_at` never touches it - so a naturally-expired temp ban that nobody manually unbanned used to keep showing up marked "Active" forever. `isCurrentlyInEffect` is the actually-correct check (not revoked **and** not time-expired), already existed on `Ban` for exactly this, just wasn't being used here.
- **`/banhistory <player>`** (Moderator+, user-requested) - the deliberate opposite of the Bans page above: a full record (active, expired, and revoked alike, capped at 25 entries) for staff who need the whole trail, not just what's currently in effect. New `BanDao#findHistoryByUuid`/`BanService#history` (`target_uuid = ?`, no active/expiry filter, most recent first) backing it - `punish.PunishModule#executeBanHistory` resolves the name to a UUID the same way `/ban`/`/tban` already do (`resolveUuid`), then renders each row's type/status/reason/actor/created/expiry, resolving `banned_by`/`revoked_by` UUIDs to last-known names via `PlayerDao#findByUuid` (falls back to the raw UUID if never seen).
...
- **`investigate/InvestigateModule#gcmd`** + `/gcmd <player> <command...>` (Senior Admin; user-requested TFM `Command_gcmd` port, patched the way TFM's own real source patches it): "run a command as another online player," via `Bukkit.dispatchCommand`. **The same class of bug the auto-op fix above closes, from a different angle**: `PlayerCommandPreprocessEvent` (what `protect.CommandAccessRegistry`'s enforcement actually hooks) structurally never fires for a command dispatched programmatically - any feature that synthesizes a command on a player's behalf bypasses `protect.command-access` entirely unless it explicitly re-checks first. `gcmd` re-validates the outgoing command against `CommandAccessRegistry#matchRawCommand` (a new, Bukkit-free method added for exactly this, alongside the existing listener-based check - kept as two separate code paths rather than refactoring the tested listener) before dispatching, checked against **the sender's** rank rather than the target's - matches TFM's real behavior, since impersonating a lower-rank player for testing/administration is the command's whole legitimate purpose, so gating on the target would defeat it; gating on the sender (already top-tier) still closes the real gap, since only an unconditional "nobody" rule can still block them.
...
- **`scoreboard/ScoreboardModule`** - configurable, independently toggleable (`modules.scoreboard.enabled`) sidebar scoreboard, MiniMessage title + lines (`{online}`/`{max}` placeholders), refreshed every 5s alongside the tab list footer. One shared `Scoreboard`/`Objective` rather than per-player (content is server-wide, cheaper to keep in sync). **Rewritten (user-reported): `Score#customName(Component)` doesn't work for this project's 1.8.8/1.12.2 Eaglercraft clients** - a real server screenshot showed the sidebar literally reading "rigel_line_0", "rigel_line_1", ... (the raw internal entry key, not the configured text). Root cause: `customName` relies on a scoreboard packet field that only exists starting with the 1.20.3 protocol rewrite - a client (or the proxy/bridge translating an Eaglercraft connection down to the real legacy wire protocol it speaks) that doesn't understand that field falls back to showing the entry itself. `scoreboard/ScoreboardService` now uses the original, universally-supported mechanism instead: each line gets its own `Team` whose prefix/suffix carries the real text (split at the pre-1.13 16-character-per-field limit, cutting only on a color-code boundary and carrying the active color across the cut via Bukkit's own `ChatColor#getLastColors`), assigned to a unique **invisible** entry (a 2-character color-code pair, `invisibleEntry`) so only the prefix/suffix render - the classic cross-version scoreboard-plugin trick, restored.
...
- **`world/RegularWorldWipeService`** + `/wipeworld confirm` (user-requested) - wipes and regenerates the **primary Overworld itself**, not flatlands or the guild plot world. Deliberately the most locked-down command in the plugin: console/RCON only with no in-game mode at all (unlike `/wipeflatlands`'s dual mode - there's no "disposable sandbox" framing to fall back on for the world people actually live in), a two-step `/wipeworld` → `/wipeworld confirm` confirmation, no auto-wipe schedule of any kind, and scoped to exactly `Bukkit.getWorlds().get(0)` - the Nether/End are never touched. Mirrors `FlatlandsService`'s in-place wipe shape (evacuate, unload, delete the folder off the `dbExecutor`, recreate, evacuate back) with one structural difference forced by the world being wiped *also* being the usual evacuation target: players move to another currently-loaded world (preferring flatlands, since it's already a safe holding area this project manages) for the duration, then land on the fresh world's own spawn once regeneration completes. Regenerates with a fresh random seed via plain `WorldCreator` - no CleanroomGenerator/flatland-style flat params, since this is meant to be real, playable terrain.
- **`world/SpawnJoinListener`'s first-join fallback** (user-requested: "make sure players spawn on the flatlands only") - previously, a genuinely brand-new player joining before any `/setspawn` had ever been run was silently left at vanilla's own primary-world spawn (`spawnService.spawnLocation()` returned empty, so the deferred teleport call was simply a no-op on an empty `Optional` - confirmed by tracing the code, not assumed). `onJoin` now falls back to `FlatlandsService#world()`'s spawn location whenever the RMCM spawn is unset **and** this is a genuine first join (`!hasPlayedBefore()`) - deliberately not applied to every "always"-policy join with no spawn set, so a returning player already playing in the main world is never silently redirected out of it. If an admin has run `/setspawn`, that explicit location is used exactly as before - this is a fallback layered on top of the existing `send-on-join` policy gating, not a replacement for it. Degrades gracefully (leaves the player where vanilla put them, same as before this fix) if flatlands itself isn't loaded yet at that exact instant - `FlatlandsService#initializeWorld()` is async (a DB read plus a scheduled-task hop, unlike `AdminWorldService#ensureWorldExists()`'s synchronous main-thread creation), so there's a narrow, non-zero window right after boot where this is possible.
- **`world/MainWorldLockdownGuard`** (user-requested: "on join, make sure player spawns in flatlands, do not allow in regular world") - keeps every non-staff player out of the primary Overworld entirely, sending them into the flatlands sandbox on join, on respawn, and any time they otherwise end up there. Scoped to `Bukkit.getWorlds().get(0)` only: the Nether/End (which `/world` exists to reach), flatlands, the guild plot world, and the admin world are all untouched. Staff bypass via `world.main-world-lockdown.bypass-rank` (default `moderator`) - without it nobody could moderate the main world and `/wipeworld`/`/setspawn` would be unusable there. Modelled directly on the existing `AdminWorldTeleportGuard` + `AdminWorldPresenceGuard` pair and layered the same way, because no single hook is provably enough: a teleport hook (which covers portals for free, since `PlayerPortalEvent extends PlayerTeleportEvent`, and **redirects rather than cancels** so a player is never left standing in a re-firing portal), a respawn hook (a death respawn is not a teleport), a `PlayerChangedWorldEvent` hook (catches the vehicle-riding case Bukkit does not reliably fire a teleport event for - the real, previously-reported "stay in a boat" exploit against the admin world), a delayed join check (the same async-rank-cache race `AdminWorldPresenceGuard` documents - checking immediately would read an unpopulated `PermissionGate` cache and eject legitimate staff on login), and a 5s periodic sweep as the final backstop. Degrades to a complete no-op if the flatlands world is not loaded, rather than stranding anyone. **Operational note**: with this enabled, `/setspawn` should be set inside flatlands - a spawn point in the Overworld just gets the player bounced straight back out.
- **`/world <nether|end>`** (user-requested, open to everyone) - fast-travel to either dimension without a portal, resolved via `Bukkit.getWorlds()` filtered by `World.Environment` (no existing helper for this anywhere in the codebase before now). Lands the player at a genuinely safe spot, not a raw `World#getSpawnLocation()` call: the End's own default spawn (a small obsidian platform) is always safe and passes immediately, but the Nether has no equivalent guarantee and can otherwise drop a player into solid netherrack or a lava sea. `WorldModule#findSafeLocation` is a bounded scan (own column first, then vertically, then a small ring of nearby columns) for two non-liquid passable blocks over a solid, non-liquid floor - falls back to the raw spawn location with an in-chat warning, rather than refusing the command outright, if nothing safe turns up within bounds. No existing "safe teleport" utility existed anywhere in this codebase to reuse - every other teleport here either replays a previously-stored `Location` or a custom-flat-world's own already-known-safe ground.
- **`announce/AnnounceModule`** — a rotating broadcaster (`announce.broadcast.messages`, one per `interval-minutes`) plus a one-off `/announce <message>` command (Moderator+). "Configurable with colors" is MiniMessage throughout (`<red>`, `<gradient:...>`, etc.), both for the configured rotation list and the command argument, rather than legacy `&`-codes. **User-requested: bare `https://`/`http://` URLs are automatically made clickable** (`AnnounceModule#linkify`) in both the rotation messages and `/announce`'s own argument - applied via Adventure's `Component#replaceText`, which matches against the already-parsed component tree rather than the raw MiniMessage source string, so a URL can never be misread as (or break) MiniMessage tag syntax, and any formatting already wrapping the URL text is preserved on the parts that aren't the link itself. **Fixed a real production crash this introduced**: MiniMessage is non-strict by default, so an unresolvable/misspelled closing tag (e.g. a typo'd `</aqla>`) isn't rejected - it's left behind as literal text; a URL sitting directly against one with no whitespace in between had that stray tag text swallowed into the "URL" match, producing a string `ClickEvent.openUrl` accepts without validation but that later blew up `Error sending packet clientbound/minecraft:system_chat` for every recipient when Paper's packet encoder called `URI.create` on it. Two independent layers now guard against this: `URL_PATTERN` excludes `<`/`>` throughout (not just the existing trailing-punctuation exclusion), and `linkify` itself calls `URI.create` and falls back to plain, still-visible (non-clickable) text on failure rather than ever handing a bad string to `ClickEvent`. Covered by `AnnounceModuleUrlPatternTest`.
...
- `protect/` — `BlockedBlockRegistry`, `BlockedEntityRegistry`, `BlockedEffectRegistry`, `CommandAccessRegistry` (TFM-syntax-compatible tiered command access - see its own javadoc), `AntiNukeService`, `AntiSpamService`, `MovementValidator`, `bridge/WorldGuardBridge` (admin-defined protected zones, e.g. spawn — reuses WorldGuard's region engine rather than inventing region storage; **independent of** the global anti-nuke/blocking primitives above, which must still apply even to OPs who'd normally hold a WorldGuard region-bypass permission — this is explicitly the gap TFM-style plugins exist to cover, so the two layers coexist on purpose, not redundantly). `protect/area/` — `/protectarea` (own DB-backed player-facing region system, not built on `WorldGuardBridge` above - see "Protected areas: /protectarea" section). `protect/worldedit/` — `WorldEditAbuseGuard` (command-preprocess abuse checks) plus `worldedit/extent/` (the real `compileOnly` WorldEdit/FAWE dependency: `EditSessionEvent` extent chain giving `/protectarea` per-block enforcement against a live edit, and the selection-volume/container/blocked-type caps - see "Protected areas: /protectarea" section). `WorldEditLimitOverrideService` + `/worldeditlimit radius|volume <value>|reset` (Senior Admin, user-requested) lets the radius/volume caps be raised or lowered at runtime without editing `config.yml` - in-memory only, never written back to disk (that file is hand-commented throughout this project; `FileConfiguration#save()` would silently strip every comment in it), so it resets to the config default on restart. Staff (Moderator+) already bypass both caps entirely regardless of this command's setting - that floor predates this addition. User-reported and fixed alongside it: `checkRadius`'s regex previously matched a whole argument token with `.matches()`, which never matches a comma-separated multi-axis radius (`//hsphere stone 50,50,50`, valid WorldEdit syntax for a non-uniform ellipsoid) - silently skipping the check entirely for that argument; it now splits on `,` and checks every sub-number. **`protect.worldedit.spawn-protection`** (user-requested: "don't allow massive WorldEdit commands near spawnpoint") applies much tighter `max-radius`/`max-blocks` caps than the general ones above, but only within `radius` blocks of the live `/setspawn` point, split across both tiers the same way the general caps are: `WorldEditAbuseGuard#checkRadius` swaps in the tighter radius cap as a cheap command-preprocess pre-check (active even without WorldEdit/FAWE's extent-chain tier attached), and `worldedit.extent.SpawnProtectionExtent` enforces the tighter block-count cap per-block-position inside the real edit session - counting only blocks that actually land within `radius` of spawn, regardless of the edit's overall selection size or where it originates, so a huge selection that merely *overlaps* the zone can't smuggle a massive edit through by anchoring itself outside it. Moderator+ still bypass both entirely, same floor as everywhere else on this page. Two explicit cleanup commands live here, both config-filterable (include/exclude lists by `EntityType`) and tick-budgeted so a large purge on a busy server doesn't spike the main thread:
...
- **Visible road/border generation, PlotSquared-style** (user-requested - the protection
  ...
  **User-reported, fixed - the spawn-chunk blind spot in that same "after they're attached"
  rule**: Bukkit generates a world's spawn-chunk region *during* `WorldCreator#createWorld()`
  itself (and `GuildPlotWorldService#ensureWorldExists`'s `getHighestBlockYAt(0, 0)` call on
  the following line forces the origin chunk too), so those chunks always predate the
  populator no matter how early it is attached - they kept bare, road-less terrain forever.
  It surfaced as *exactly one* broken plot corner, the intersection square nearest world
  origin on the very first plot, with every other corner in the world correct - which is what
  distinguished it from the `PlotWorldTerrain` corner bug below, a classification error that
  by definition broke *every* intersection identically. `GuildPlotWorldPopulator#populate`'s
  body was extracted into a static `paintChunk(WorldInfo, RegionAccessor, ...)` - written
  against `RegionAccessor`, the common supertype of both `LimitedRegion` (what a populator
  is handed) and `World` (a plain live world) - and `ensureWorldExists` now runs that same
  method, not a reimplementation, over exactly `World#getLoadedChunks()` immediately after
  attaching the populator, which was assumed to be precisely the pre-generated set. **That assumption was wrong, and
  the bug was reported a second time**: modern Paper does not keep spawn chunks loaded, so
  they are generated, written to disk, and unloaded again - leaving that sweep with nothing
  to do, and the gap visibly spanning several chunks rather than one 7-wide intersection
  square. `repairPreGeneratedChunks` now asks the authoritative question instead -
  `World#isChunkGenerated` (which never generates anything itself) over a bounded radius
  around origin (`guild.plotworld.spawn-repair-radius-chunks`, default 12, 0 disables) -
  and repaints only chunks that genuinely already exist, skipping ones not yet generated
  since the populator handles those correctly on its own. It runs on **every** enable, so a
  plot world predating the fix self-heals on the next restart rather than needing a
  `/wipeguildplots confirm`; that is only safe because `paintChunk` was made **idempotent**
  (it detects an already-painted border column and leaves it alone - without that, each run
  would read the *wall's* Y as ground level via `getHighestBlockYAt` and stack another wall
  a block higher every boot). Tick-budgeted at 8 chunks/tick, and plot interiors are never
  touched - only `ROAD`/`BORDER`/`INTERSECTION` columns, which live under the boundary
  lockdown anyway.
- **Repaired chunks must produce the same block *states*, not just the same blocks** (user-reported): after the repair pass landed, walls near world origin looked visibly different from walls further out, with the repair radius boundary plainly marking where one style became the other. Same material, same position - different state. A populator's `LimitedRegion` writes never trigger neighbour updates, so a cobblestone wall keeps its default "post" state and a run of them renders as separate crenellations; `RegionAccessor#setType` on a live `World` applies physics, connecting each wall to its neighbours and collapsing that run into one low continuous wall. `GuildPlotWorldPopulator#write` now routes the live-world path through `Block#setType(Material, boolean)` with physics disabled, so both passes emit identical states. `#matchGeneratorState` additionally resets any wall an *earlier* repair pass already connected, so an existing plot world converges without a `/wipeguildplots confirm`; it only ever touches a wall that is actually connected to something (the generator never produces one), so the common path stays a single block-data read.
- **`PlotWorldTerrain.CellType.INTERSECTION`** is a fourth cell type split out of `BORDER`
  (user-reported, after seeing the sealed corner in-game): sealing the crossing worked, but
  capping the whole `plot-gap x plot-gap` square with the *wall* block rendered as a bumpy
  lattice of disconnected fence posts rather than a clean crossing, because a wall block is
  a thin connecting post and not a full cube. Straight one-column borders still use the wall
  block - that is exactly what it suits - while an intersection is now capped with a solid
  block. Both are built by the same `paintPillar` helper, differing only in the cap. The
  whole palette also became configurable in the same pass
  (`guild.plotworld.materials.{road,border-foundation,border-wall,intersection}`, resolved by
  `PlotWorldMaterials#fromConfig`, which falls back per-slot and logs a warning on an
  unrecognised block name rather than failing world creation over one cosmetic typo). Default `plot-gap` is 7
  (one border column each side, five open road columns between) - user-reported the
  original 16-wide default read as an oversized road once actually visible.
  **User-reported, fixed**: the first version painted roads/borders at the *configured*
  `guild.plotworld.ground-y` unconditionally, which produced an entire road network
  floating in mid-air over the real terrain on a live server - the optional CleanroomGenerator
  plugin (see `world.CleanroomGeneratorBridge`) wasn't installed, so plot-world creation
  silently fell back to bare vanilla `WorldType.FLAT` with no matching `generatorSettings`,
  generating at Bukkit's own default flat height, essentially never the same Y as whatever
  `ground-y` happens to be configured to - a config value nothing was actually enforcing.
  `GuildPlotWorldPopulator` now reads each column's *real* generated height via
  `LimitedRegion#getHighestBlockYAt` instead of trusting config at all, and
  `GuildPlotWorldService#plotTeleportLocation`/`ensureWorldExists`'s spawn-location call
  both switched from `ground-y + 1` to `World#getHighestBlockYAt(...) + 1` the same way -
  correct regardless of which generator actually produced the world. `ground-y` is now used
  for exactly one thing: centering a plot's fixed-band *protection* column (see this
  class's own javadoc on why that's a deliberately generous, Bukkit-free band rather than a
  live-height query) - never terrain placement or teleport destinations.
- **Border walls run from the surface to bedrock** (user-requested): `GuildPlotWorldPopulator`
  fills every border column solid from `WorldInfo#getMinHeight()` up through ground level,
  not just a one-block footing under the visible wall block - a genuine foundation, not a
  thin shell.
- **`/guild admin plotbypass`** (Senior Admin, in-game only; user-requested): nobody can
  build/break outside their own plot in the plot world - not even the border wall itself,
  and not even staff - without a Senior Admin first toggling this on for themselves.
  `guild.plot.GuildPlotBoundaryGuard` enforces it as its own additive listener at
  `EventPriority.HIGH` for `BlockBreakEvent`/`BlockPlaceEvent` only (explosions, buckets,
  pistons, hanging entities, and WorldEdit/FAWE edits inside the plot world still fall back
  to `protect.area.ProtectAreaService#isBypassing`'s existing Moderator+ floor, unchanged -
  not extended here). Deliberately implemented as a second, independent listener rather
  than a change to that shared floor, which every other `/protectarea` region project-wide
  still relies on exactly as before - this is the one place in the whole plugin where
  "Moderator+ silently bypasses everything" isn't enough on its own; every other protected
  region is unaffected. `guild.plot.GuildPlotBypassService` is the session-only per-player
  toggle state (same "always resets on relog" shape as `vanish.VanishService`).
...
- **Anti-nuke**: in-memory sliding-window counters (`ConcurrentHashMap<UUID, RateWindow>`) over block break/place/explode events — synchronous since cancellation must happen sync, no DB round-trip on the hot path. Breach → cancel, optional auto-freeze, staff notify, async audit-log write. Applies regardless of WorldGuard region bypass or OP status — see `protect/` note above. **`protect/antigrief/ProjectileSpamGuard`** (new, user-reported: a real Watchdog thread dump traced a server freeze to `ThrowableProjectile.tick()`'s per-tick nearby-entity scan, with nothing anywhere in this codebase previously rate-limiting how many projectiles - ender pearls, snowballs, eggs, potions - a single player could launch per second) applies the exact same sliding-window shape to `ProjectileLaunchEvent`, reusing `PerPlayerRateLimiter` and `AntiGriefSupport#applyAction` unchanged. Only ever throttles a player's own throws (`Projectile#getShooter() instanceof Player`) - dispensers/mobs launching their own projectiles are untouched.
- **Command blocks** (`protect/antigrief/CommandBlockGuard`, user-requested after a live incident: a repeating command block running `/give all netherite_axe` fired hundreds of times a second and handed the whole server netherite). **This closed a wide-open hole, not a tuning gap**: `protect.command-access` is enforced by `protect/BlockedCommandListener`, whose only hook is `PlayerCommandPreprocessEvent` and which calls `event.getPlayer()` unconditionally - so it is structurally player-only, and a command block bypassed *every* command-access rule, including unconditional `n:b:` "nobody" ones. Repo-wide grep confirmed zero prior handling of `ServerCommandEvent`, `BlockCommandSender`, `CommandMinecart`, or `Material.COMMAND_BLOCK`. It matters doubly here because every player is auto-op'd (`core/AutoOpModule`), so anyone could place and program one. Three hooks: `ServerCommandEvent` (cancel, then delete the block one tick later - a repeating command block fires every tick, so cancelling alone would leave the server rejecting it forever, whereas deleting also self-cleans command blocks placed before the guard existed), `BlockPlaceEvent` (refuse placement of all three command-block variants, plus `STRUCTURE_BLOCK`/`JIGSAW` under `block-structure-blocks`), and `EntitySpawnEvent` (command minecarts are entities, so `BlockPlaceEvent` never sees them). TFM ref: `blocking.command.CommandBlocker#onServerCommand`, which hooks the same event and tests the same sender pair - there is no dedicated command-block event in the Paper API (verified against the 26.1.2 jar). **Two details are load-bearing rather than stylistic**: (1) the sender test is a positive `instanceof BlockCommandSender || instanceof CommandMinecart` allowlist, *not* "anything that isn't a player", because `RemoteServerCommandEvent extends ServerCommandEvent` - the naive form would silently break **RCON**, plain console, and every plugin-dispatched command (`/store grant`, `/vote record`); (2) staff alerts are throttled to one per command block per `alert-interval-seconds`, because `AntiGriefSupport#notifyStaff` does no throttling of its own and a per-execution alert would itself be the lag bomb the guard exists to stop (TFM ships `log_interval_ticks` for the same reason). No staff exemption - and one could not work for execution anyway, since a command block has no owner to check a rank against. The three command-block ids plus structure/jigsaw were also added to `protect.worldedit.blocked-block-types` as defense-in-depth, though that list is WorldEdit-only and exempts Moderator+, so the execution guard is the actual fix.
- **Fragile ("pops into an item") blocks in WorldEdit** (`protect/worldedit/FragileBlockPolicy` + `extent/FragileBlockExtent`, user-requested). The mirror image of the gravity blocks already in `blocked-block-types`: sand/gravel/anvils become a mass `FallingBlock` spawn when gravity triggers, while saplings, carpets, torches, flowers, redstone and rails cannot support themselves and become a mass `Item` spawn the moment their support changes - a `//set torch` over a large region is a lag bomb primed on the floor beneath it. **Two tiers**: the high-risk subset (redstone wire/repeaters/comparators, all rails, all pressure plates, tripwire - the ones that additionally drive redstone tick churn or fire on entity contact) is refused outright via `BlockedTypeExtent`'s `MaxChangedBlocksException(0)` idiom, and everything else fragile is *capped* per operation (`max-per-operation`, default 1000) via `ContainerLimitExtent`'s counting idiom. Capping rather than banning the broad tier is deliberate: almost every schematic contains a few torches or flowers, so an outright ban would stop non-staff pasting essentially any build. Moderator+ exempt, sharing the extent chain's single existing staff gate. **Deliberately no hand-maintained block list** - classification comes from WorldEdit's own registry (`BlockMaterial#isFragileWhenPushed()`, i.e. Minecraft's `PushReaction.DESTROY`, plus `BlockCategories` which wraps Minecraft's block tags), so new blocks are covered automatically on a Minecraft update; this is the same shape as the `hasContainer()` flag already backing the container cap, and it honours rather than reverses `ContainerLimitExtent`'s javadoc, which explicitly argued against TFM's "needs updating every Minecraft version" list approach. Two structural notes: the classifier consumes a `BlockStateHolder` (what the extent already holds, and what `BlockCategory#contains` is declared over) so no `BukkitAdapter` or Bukkit `Material` crossing is needed - the extent package has never needed one; and the pure boolean decision lives in a separate, WorldEdit-import-free `FragileBlockRule` because `worldedit-core` is `compileOnly` and therefore absent from the test runtime classpath, so a test cannot even *load* a class whose signatures mention `BlockStateHolder`. That split is what makes any of it testable. **Not** wired into `WorldEditAbuseGuard`'s command-preprocess layer, and that is a correctness constraint rather than a scope cut: that listener registers unconditionally even when WorldEdit is absent, so a hard reference to `BlockTypes`/`BlockCategories` there would throw `NoClassDefFoundError` on a server without WorldEdit installed. The extent layer only loads when WorldEdit is actually present and is strictly more thorough anyway, catching `//paste`, brushes and generation commands a string check over command arguments cannot see.
...
- **Crash-exploit guards** (`protect/crash/`, TFM-parity "E1"/"E2" tiers): `ItemGuard`, `EntityNameGuard`, `SignContentGuard`, `SpawnerGuard` screen for oversized/malformed payload shapes at the Bukkit-event level (public API only, no NMS reflection), plus `crash.packet.CrashPacketListener` (soft-dependency on the standalone PacketEvents plugin) for inbound chat/command/movement packet-flood rate limiting and outbound entity-metadata sanitization. `ItemGuard#onSwapHands` specifically defends against the "Decimator" exploit (user-reported): a crafted/bloated item rapid-fired between main hand and offhand via a client-side script, which forces an expensive equipment-sync packet broadcast to every nearby observer on each swap — closed two ways, both independent of PacketEvents being installed at all: the same oversized-content check `ItemGuard` already applies everywhere else (a real gap before this fix — hand-swaps never fired `InventoryClickEvent`/`InventoryDragEvent`), plus a dedicated per-player swap-rate limit (`protect.crash.items.max-hand-swaps-per-second`, default 6) so even a "clean" item can't be swapped fast enough to matter. Moderator+ exempt throughout, matching every other guard in this tier.
  - **User-reported real server freeze, root-caused and closed**: a live Watchdog thread dump showed the main thread stuck decoding a chest's contents (`ChestBlockEntity.loadAdditional`/`ContainerHelper.loadAllItems`, a deep Mojang-codec decode chain) - triggered by CoreProtect's own interact-logging forcing a synchronous `CraftBlock#getState()` when the chest was right-clicked. That decode cost is unavoidable once something needs to read the block, so the only real fix is stopping a cursed item from ever reaching a placed container - not detecting it after the fact. Two paths let that happen without `ItemGuard`'s existing book-size/container-nesting checks ever running: editing a book directly (fires `PlayerEditBookEvent`, not `InventoryClickEvent`/`InventoryDragEvent` - the log showed the player repeatedly hitting vanilla's own hardcoded "Book edited too quickly!" *rate* limit on exactly this unguarded *content* path) and a hopper/dropper auto-transferring an item between containers (fires `InventoryMoveItemEvent`, also unhooked). Both are now hooked with the exact same checks every other `ItemGuard` path already uses. `isCursed` also gained a total-nested-item-**count** cap (`protect.crash.items.max-nested-item-count`, default 200) alongside its existing **depth** cap (`max-container-depth`) - many sibling containers at an allowed depth could otherwise still add up to an expensive decode without ever tripping the depth check. Deliberately not attempted: a WorldEdit-extent NBT-content screen for blocks placed via `//set`/`//replace` (the player's first logged command in this incident) - `worldedit/extent/`'s existing `ContainerLimitExtent` only caps container block *count*, not per-block NBT content; closing that fully is a larger, separate follow-up.
