...
- `webpanel/` — opt-in, off-by-default, no-auth (relies on `bind-address: 127.0.0.1`-by-default instead - see `WebPanelServer`'s own javadoc), read-only HTTP dashboard (stats/bans/permban cases, staff roster showing a player's highest-held title in place of "Senior Admin" when they hold one, and a schematics browse/download page for WorldEdit/FAWE files - `SchematicsService` has no upload endpoint at all, by design, and its path resolution is the one place this whole panel does anything security-sensitive - see its own javadoc). See Ban system section for the full security rationale.
